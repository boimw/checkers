...
{ 
    static class Utils
    {
        ...
        public static Move[] GetOpenSquaresBlack(Square[,] Board, Point checker)
        {
            return GetOpenSquaresBlack(Board, checker, new Move(-1, -1, -1, -1), null);
        }

        private static Move[] GetOpenSquaresBlack(Square[,] Board, Point checker, Move lastMove, List<Point> priorPositions)
        {
            if (priorPositions == null)
            {
                priorPositions = new List<Point>();
            ...
            //Dole desno
            if (Board[priorPositions[0].Y, priorPositions[0].X].Colour != CheckerColour.Black || Board[priorPositions[0].Y, priorPositions[0].X].King)        //Stop regular red pieces from moving backwards
            {
                if (IsValidPoint(checker.X + 1, checker.Y + 1))
                {
                    if (Board[checker.Y + 1, checker.X + 1].Colour == CheckerColour.Empty && lastMove.Destination.X == -1)  //Allow immediate empty spaces if it's the first jump
